...
// 		// At this point, we know response is not empty
// 		const user = response[0]; // Get the first (and should be only) user

// 		if (user.state !== 'verified' && user.state !== 'notverified') {
// 			if (user.state === otp) {
// 				QUERY("UPDATE user SET state='verified' WHERE user_name='" + email + "'").then(() => {
// 					res.send({type: 'success', message: 'Account verified successfully'});
// 				});
// 			} else {
// 				res.send({type: 'error', message: 'Invalid OTP'});
// 			}
// 		} else {
// 			res.send({type: 'error', message: 'An error occurred'});
// 		}
// 	}).catch((error) => {
// 		console.error('Error in /verifyuser:', error);
// 		res.send({type: 'error', message: 'An error occurred. Try again later.'});
// 	});
// });
